...
class BayesianOptimization2:
    ...
    def black_box_function(self, config, simulateMotionSensors = True, simulateEstimotes = False, simulateIS = False, Plotting = False):       
        sensorPositions = []
        sensorTypes = []
        sensor_xy = []
        excluded = []
    ...
    def function_to_be_optimized(self, trial):

        variables = self.BuildConfigurationSearchSpace(trial)

        return 100 - self.black_box_function(variables, 
    ...
    def BuildConfigurationSearchSpace(self, trial):
        list_of_variables = {}
        if (self.LSsensorTypesNum > 0):
            for i in range(1, self.CONSTANTS['max_LS_sensors'] + 1):

                x = trial.suggest_int("x" + str(i), 1, int((self.CONSTANTS['width'] - 1) / self.CONSTANTS['epsilon']))

                y = trial.suggest_int("y" + str(i), 1, int((self.CONSTANTS['width'] - 1) / self.CONSTANTS['epsilon']))

                if self.LSsensorTypesNum > 1:
                    t = trial.suggest_int("t" + str(i), 1, self.LSsensorTypesNum)

                else:
                    t = trial.suggest_int("t" + str(i), 1, 1)

                #list_of_variables.append(x)
                #list_of_variables.append(y)
                #list_of_variables.append(t)
                list_of_variables["x" + str(i)] = x
                list_of_variables["y" + str(i)] = y
                list_of_variables["t" + str(i)] = t
        ...
        if (self.ISsensorTypesNum > 0):
            for i in range(1, self.CONSTANTS['max_IS_sensors'] + 1):
                ...
                objects_location = trial.suggest_categorical('object_location' + str(i), objects)

                t_o = trial.suggest_int("t_o" + str(i), 2 + 1, 2 + self.ISsensorTypesNum)

                #list_of_variables.append(objects_location)
                #list_of_variables.append(t_o)
                list_of_variables["objects_location" + str(i)] = objects_location
                list_of_variables["t_o" + str(i)] = t_o
    ...
    def run(self):
        ...
        study.optimize(self.function_to_be_optimized, n_trials = self.CONSTANTS["iteration"])
