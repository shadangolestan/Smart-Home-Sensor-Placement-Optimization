...
class GreedyAndLocalSearch:
    ...
    def RunGreedyAlgorithm(self):
        ...
        while picked_sensors < self.maxSensorNum and CONSTANTS['iterations'] > 0:   
            print('Number of sensors picked: ', len(self.best_configuration_history))
            self.test_configs = []
...
def run(run_on_colab = False, 
    ...
    population = int(((CONSTANTS['height'] / epsilon) - 1) * ((CONSTANTS['width'] / epsilon)) - 1)
